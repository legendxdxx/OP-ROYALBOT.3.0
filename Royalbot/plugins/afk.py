...
from royalbot.sql.gvar_sql import addgvar, gvarstat, delgvar
from . import *
...
@R1.on(events.NewMessage(outgoing=True))
async def set_not_afk(event):
    ...
    if "#" not in current_message and gvarstat("AFK") == "YES":
        royalbot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            await unsave_gif(event, royalbot)
            delgvar("AFK")
            ...
            await event.client.send_message(
                ...
                + f" Ask in {royal_grp} to get help!",
...
@R1.on(events.NewMessage(incoming=True, func=lambda e: bool(e.mentioned or e.is_private)))
...
@R1.on(admin_cmd(pattern="afk(?:\s|$)([\s\S]*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    owo = event.text[5:]
    reason_1 = owo
    royalpic_1 = await event.client.download_media(krakenop)
    if gvarstat("AFK") != "YES":
        ...
        if owo == "":
            ...
            addgvar("AFK", "YES")
            x = await event.client.send_message(
                event.chat_id, f"**I'm going afk🚶**\n\n**Because :** `{reason_1}`", file=royalpic_1)
            try:
                await unsave_gif(event, x)
                xy = await event.client.send_message(
                    Config.LOGGER_ID,
                    f"#AFKTRUE \n**AFK mode** = `True`\n**Reason:** `{reason_1}`", file=royalpic_1
                    )
                try:
                    await unsave_gif(event, xy)
                except:
...
if R2:
    @R2.on(events.NewMessage(outgoing=True))
    async def set_not_afk(event):
        global afk_time_2
        global last_afk_message_2
        global afk_start_2
    ...
    @R2.on(events.NewMessage(incoming=True, func=lambda e: bool(e.mentioned or e.is_private)))
    async def on_afk(event):
        global afk_time_2
        global last_afk_message_2
        global afk_start_2
    ...
    @R2.on(admin_cmd(pattern="afk(?:\s|$)([\s\S]*)"))
    async def _(event):
        if event.fwd_from:
            return
        krakenop = await event.get_reply_message()
        ...
        owo = event.text[5:]
        reason_2 = owo
        royalpic_2 = await event.client.download_media(krakenop)
