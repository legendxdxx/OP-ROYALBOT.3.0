...
async def logger_id(client):
    ...
    try:
        grp = await client(functions.channels.CreateChannelRequest(title="ROYALẞØT Logger", about=desc, megagroup=True))
...
<b><i>»» <u><a href='https://t.me/royalyserbot'>ROYALẞØT</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("ROYALẞØT Network", "https://t.me/RDNNETWORK")]],
    )


# Joins the ROYAL-BOT chat and channel from all clients
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@ROYALYSERBOT"))
